...
antlrcpp::Any ASTVisitor::visitReturnstmt(ifccParser::ReturnstmtContext *ctx)
{
	if (ctx->expr())
	{
		...
		currentCFG->add_to_current_bb(operation, {name_index, last_block->get_label(), type.getType()});
	}
	return 0;
}
...
antlrcpp::Any ASTVisitor::visitDeclaration(ifccParser::DeclarationContext *ctx)
{
	...
	if (ctx->expr())
	{
		...
		currentCFG->add_to_current_bb(operation, {var_type, var_index, rightExpr_type, rightExpr_index});
	}
...
antlrcpp::Any ASTVisitor::visitConstexpr(ifccParser::ConstexprContext *ctx)
{
	...
	currentCFG->add_to_current_bb(operation, {name_index, const_value, type.getType()});
	return name;
}
...
antlrcpp::Any ASTVisitor::visitAddsub(ifccParser::AddsubContext *ctx)
{
	...
	string name = currentCFG->create_new_tempvar(Type("int"), currentFunctionName);
	...
	currentCFG->add_to_current_bb(operation, {name_index, left_index, right_index});
	return name;
}
...
antlrcpp::Any ASTVisitor::visitUnaryexpr(ifccParser::UnaryexprContext *ctx)
{
	...
	currentCFG->add_to_current_bb(operation, {name_index, expr_index});
	return name;
}
...
antlrcpp::Any ASTVisitor::visitFunctiondef(ifccParser::FunctiondefContext *ctx)
{
	...
	if (ctx->defParams())
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			currentCFG->add_to_current_bb(operation, {registers[i], to_string(currentCFG->get_symbol_table_index()[varName])});
		}
	}
	visitChildren(ctx);
	BasicBlock *last_block = currentCFG->get_last_bb();
...
antlrcpp::Any ASTVisitor::visitCallexpr(ifccParser::CallexprContext *ctx)
{
	...
	currentCFG->add_to_current_bb(operation, {"eax", returnVar_index});
...
antlrcpp::Any ASTVisitor::visitCallFunction(ifccParser::CallFunctionContext *ctx)
{
	...
	if (ctx->args())
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			currentCFG->add_to_current_bb(operation, {to_string(currentCFG->get_symbol_table_index()[arg]), registers[i]});
		}
	}
	currentCFG->add_to_current_bb(new Call(), {funcName});
	return funcName;
}
...
antlrcpp::Any ASTVisitor::visitPutchar(ifccParser::PutcharContext *ctx)
{
	...
	currentCFG->add_to_current_bb(operation, {name_index});
...
antlrcpp::Any ASTVisitor::visitIf_stmt(ifccParser::If_stmtContext *ctx)
{
	...
	for (int i = 0; i < blocks.size(); i++)
	{
		...
		if (i < exprs.size())
		{
			...
			test_bb->add_IRInstr(operationCmp, {expr_name_index}); // cmp expr to 1 if eq jump
			Operation *operationJmpNotEqual = new JmpNotEqual();
			test_bb->add_IRInstr(operationJmpNotEqual, {then_bb->get_label()});
		}
		else
		{
...
antlrcpp::Any ASTVisitor::visitWhile_stmt(ifccParser::While_stmtContext *ctx)
{
	...
	string expr_name_index = to_string(currentCFG->get_symbol_table_index()[expr_name]);
	Operation *operationCmp = new Cmp();
	endwhile_bb->add_IRInstr(operationCmp, {expr_name_index}); // cmp expr to 1 if eq jump

	Operation *operationJmpNotEqual = new JmpNotEqual();
	endwhile_bb->add_IRInstr(operationJmpNotEqual, {then_bb->get_label()});
